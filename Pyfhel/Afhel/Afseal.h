...
class Afseal {
  ...
  */
  ...
  //SETTERS
  void setpublicKey(PublicKey &pubKey) { this->publicKey = make_shared<PublicKey>(pubKey); }
  void setsecretKey(SecretKey &secKey) { this->secretKey = make_shared<SecretKey>(secKey); }
  void setrelinKey(RelinKeys &relKey) { this->relinKey = make_shared<RelinKeys>(relKey); }
